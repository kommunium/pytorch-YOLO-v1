...
def decoder(pred):
    ...
    mask1 = contain > 0.1  # thresholding
...
def nms_(bboxes, scores, threshold=0.5):
    ...
    while order.numel() > 0:  # torch.numel() returns the number of elements
        if order.numel() == 1:  # maintain one box
            i = order.item()
            keep.append(i)
            break
        else:
            i = order[0].item()  # maintain the box[i] with the greatest score
            keep.append(i)

        # calculate the IOU between box[i] and other boxes
        ...
        idx = (iou <= threshold).nonzero().squeeze()  # note that now idx is [N-1,] while order is [N,]
        if idx.numel() == 0:
            break
        order = order[idx + 1]  # fix the difference between indices
    return torch.LongTensor(keep)  # indices of pytorch is LongTensor
...
def predict_gpu(model, image_name, root_path=''):
    ...
    for i, box in enumerate(boxes):
        ...
        prob = probs[i]
        prob = float(prob)
        result.append([(x1, y1), (x2, y2), PART_CLASSES[cls_index], image_name, prob])
    return result
...
if __name__ == '__main__':
    ...
    image_name = '0000180.jpg'
    image = cv2.imread(image_name)
    print('predicting...')
    result = predict_gpu(model, image_name, root_path='../../data/VOC2007/JPEGImages/')
    print(result)

    for left_up, right_bottom, class_name, _, prob in result:
        color = Color[PART_CLASSES.index(class_name)]
        ...
        cv2.putText(image, label, (p1[0], p1[1] + baseline),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.4, (255,) * 3, 1, 8)
